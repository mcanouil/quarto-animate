...
- build: Update the vendored Lua modules to 2.5.0, which adds the schema accessors and the `schema-check` fix for an extension whose entry points are in a subdirectory. The schema validator moves to its own release train, pinned at `schema-v2.2.0`, which accepts only `true` and `false` as a boolean. (#71, #73)
